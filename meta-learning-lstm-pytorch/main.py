...
FLAGS.add_argument('--input-dim', type=int,
                   help="Input dimension for learner")
...
FLAGS.add_argument('--csv_train', type=str, help="Path to the training CSV file")
FLAGS.add_argument('--csv_val', type=str, help="Path to the validation CSV file")
FLAGS.add_argument('--csv_test', type=str, help="Path to the testing CSV file")
FLAGS.add_argument('--csv_eval', type=str, help="Path to the evaluation CSV file")


# Paths
FLAGS.add_argument('--data', choices=['WGM'],
...
def main():
    ...
    #learner_w_grad = Learner(args.input_size, args.hidden_size, args.n_class).to(args.dev)
    learner_w_grad = Learner(args.input_dim, args.n_class).to(args.dev)
    ...
    for eps, (episode_x, episode_y) in enumerate(train_loader):
        ...
        # train_input = episode_x[:, :args.n_shot].reshape(-1, *episode_x.shape[-3:]).to(args.dev) # [n_class * n_shot, :]
        train_input = episode_x[:, :args.n_shot].reshape(-1, episode_x.shape[-1]).to(args.dev)  # [n_class * n_shot, features]
        train_target = torch.LongTensor(np.repeat(range(args.n_class), args.n_shot)).to(args.dev) # [n_class * n_shot]
        # test_input = episode_x[:, args.n_shot:].reshape(-1, *episode_x.shape[-3:]).to(args.dev) # [n_class * n_eval, :]
        test_input = episode_x[:, args.n_shot:].reshape(-1, episode_x.shape[-1]).to(args.dev)  # [n_class * n_eval, features]
        test_target = torch.LongTensor(np.repeat(range(args.n_class), args.n_eval)).to(args.dev) # [n_class * n_eval]
